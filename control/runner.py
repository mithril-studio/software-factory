...
async def _source(boxd: AsyncBoxd, repo: str, agent: str, log: RunLog) -> str:
    ...
    if not source:
        ...
        if source:
            log.write(f"[factory] no snapshot for agent {agent!r}; falling back to {source}")
    if not source:
        source = agents.golden_name(agent)
        log.write(f"[factory] no golden snapshot for agent {agent!r}; trying machine {source}")
    return source
...
async def _execute(
    ...
    try:
        ...
        source = await _source(boxd, repo, agent, log)
