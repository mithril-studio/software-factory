...
`provision` — and every step is a status transition on that row. A run that dies mid-flight
is visible as a row stuck in a state, which is what makes the reconciler possible (§4). The
pipeline itself — budgets, criteria, labels, merge — is `../docs/architecture.md` §4.
